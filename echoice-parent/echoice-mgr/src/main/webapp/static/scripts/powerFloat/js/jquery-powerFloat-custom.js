...
(function($) {
	$.fn.powerDelCol=function(){
		$(this).live('click',function(){
			$(this).parents('tr').remove();
		});
	}
	
	$.fn.powerAddCol=function(options){
		$(this).live('click',function(){
			$(options.desc).append($(options.src).html());
		});
	}
	
	$.fn.powerLinkSelect=function(){
		$(this).live('click',function(){
			$(o.trigger).val($(this).text());
			var parentEl=$(o.trigger).parent();
			$('input[type="hidden"]',parentEl).val($(this).attr('data'));
			$(o.trigger).focus();
		});
		return this;
	}	
	
	$.fn.powerHideFloat=function(){
		$(this).live('click',$.powerFloat.hide);
		return this;
	};
	...
	var o = {
		targetGet: function() {
			...
			//$.ccmApp.trigger=this.trigger;
			$.powerFloat.trigger=this.trigger;
			$.powerFloat.target=this.s.target;
			//$(this.s.target).bgiframe();
			if(this.s.targetMode!='list'){
				$(this.s.target).bgiframe();
			}
			var attr = this.trigger.attr(this.s.targetAttr), target = this.s.target,treeUrl=this.s.treeUrl,dataType=this.s.dataType,treeParentSel=this.s.treeParentSel;
								...
								$.ajax({
									...
									success: function(data) {
										if (typeof(data) === "string") {
											...
											if ($.isArray(data) && (arrLength = data.length)) {
												$.each(data, function(i, obj) {
													var list = "", strClass = "", text, href,value;
													if (i === 0) {
													...
													if (typeof(obj) === "object" && (text = obj.text)&& (value = obj.value)) {
														if (href = (obj.href || "javascript:")) {
															list = '<a href="' + href + '" class="float_list_a" data="'+value+'">' + text + '</a>';	
														} else {
															list = text;
														}
											...
											targetHtml += '</ul>';
											if(data.length>6){
												targetHtml='<div style="height:125px;overflow: auto;">'+targetHtml+'</div>';
											}
											o.target = $('<div class="float_ajax_data">' + targetHtml + '</div>');
											o.targetShow();
											o.cacheData[url] = targetHtml;
										}
									},
									error: function() {
				...
				case "list": {
					...
					if ($.isArray(target) && (arrLength = target.length)) {
						$.each(target, function(i, obj) {
							...
							if (typeof(obj) === "object" && (text = obj.text.toString())&& (value = obj.value.toString())) {
								if (href = (obj.href || "javascript:")) {
									list = '<a href="' + href + '" class="float_list_a" data="'+value+'">' + text + '</a>';	
								} else {
									list = text;	
		...
		setWidth: function() {
			...
			if (w === "auto") {
				...
				this.target.width(this.trigger.outerWidth(true));
			} else {
	...
	var defaults  = {
		width: "inherit", //可选参数：inherit，数值(px)
		offsets: {
			x: 0,
			y: 0	
		},
